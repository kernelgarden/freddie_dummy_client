defmodule FreddieClient.PerfCollector do
  ...
  def handle_cast({:get_result, received}, state) do
    ...
    new_state = %FreddieClient.PerfCollector{
      state
      | total_received_byte: state.total_received_byte + received,
        completed_num: state.completed_num + 1
    }
  ...
  defp print_result(state) do
    IO.puts(
      "Total received bytes #{state.total_received_byte} / #{state.profiling_time}, #{
        state.completed_num
      } dummy clients"
    )
  end
